...
Libraries that can be installed through `pip3` on Python 3.7: `numpy scipy matplotlib torch keras==2.3.1 tensorflow==1.15.0 protobuf==3.20.0 imageio fire` (and potentially `wheel` for installing them, perhaps `pip3 install --upgrade pip` is needed beforehand)
